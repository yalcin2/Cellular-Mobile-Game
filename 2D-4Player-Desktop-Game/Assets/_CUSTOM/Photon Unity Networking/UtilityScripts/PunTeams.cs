...
public class PunTeams : MonoBehaviour
{
    ...
    public void UpdateTeams()
    {
        ...
        for (int i = 0; i < PhotonNetworkManager.playerList.Length; i++)
        {
            PhotonPlayer player = PhotonNetworkManager.playerList[i];
...
public static class TeamExtensions
{
    ...
    {
        if (!PhotonNetworkManager.connectedAndReady)
        {
            Debug.LogWarning("JoinTeam was called in state: " + PhotonNetworkManager.connectionStateDetailed + ". Not connectedAndReady.");
            return;
        }
