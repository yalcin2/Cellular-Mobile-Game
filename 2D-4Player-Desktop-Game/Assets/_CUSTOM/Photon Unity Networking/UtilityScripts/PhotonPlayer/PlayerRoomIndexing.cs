...
namespace ExitGames.UtilityScripts
{
	...
	public class PlayerRoomIndexing : PunBehaviour
	{
		...
		public override void OnJoinedRoom()
		{
			...
			{
				AssignIndex(PhotonNetworkManager.player);
			}else{
				RefreshData();
			}
		}
		...
		{
			if (!forceIndexing && !PhotonNetworkManager.isMasterClient)
			{
				return;
			}
			...
			if(PhotonNetworkManager.room.CustomProperties.TryGetValue(PlayerRoomIndexing.RoomPlayerIndexedProp, out _indexes))
			{
				_indexesLUT_local = _indexes as Dictionary<int,int>;
			}

			// check if we need to assign
			if (_indexesLUT_local.Count != PhotonNetworkManager.room.PlayerCount)
			{
			 	foreach(PhotonPlayer _p in	PhotonNetworkManager.playerList)
				{
		...
		void RefreshData()
		{
			if (PhotonNetworkManager.room!=null)
			{
				_playerIds = new int[PhotonNetworkManager.room.MaxPlayers];
				if (PhotonNetworkManager.room.CustomProperties.TryGetValue(PlayerRoomIndexing.RoomPlayerIndexedProp, out _indexes))
				{
		...
		void AssignIndex(PhotonPlayer player)
		{
			...
			List<bool> _indexesPool = new List<bool>( new bool[PhotonNetworkManager.room.MaxPlayers] );
			foreach(KeyValuePair<int,int> _entry in _indexesLUT)
			{
			...
			PhotonNetworkManager.room.SetCustomProperties(new Hashtable() {{PlayerRoomIndexing.RoomPlayerIndexedProp, _indexesLUT}});
