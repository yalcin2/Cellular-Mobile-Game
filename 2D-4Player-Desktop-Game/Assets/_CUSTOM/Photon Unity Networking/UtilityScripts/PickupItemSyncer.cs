...
public class PickupItemSyncer : Photon.MonoBehaviour
{
    ...
    {
        Debug.Log("Joined Room. isMasterClient: " + PhotonNetworkManager.isMasterClient + " id: " + PhotonNetworkManager.player.ID);
        // this client joined the room. let's see if there are players and if someone has to inform us about pickups
        this.IsWaitingForPickupInit = !PhotonNetworkManager.isMasterClient;
    ...
    private void SendPickedUpItems(PhotonPlayer targetPlayer)
    {
        ...
        double now = PhotonNetworkManager.time;
        ...
        for (int i = 0; i < items.Length; i++)
        {
            ...
            {
                double timeUntilRespawn = pi.TimeOfRespawn - PhotonNetworkManager.time;
                if (pi.TimeOfRespawn > soon)
                {
                    ...
                    Debug.Log(pi.ViewID + " respawn: " + pi.TimeOfRespawn + " timeUntilRespawn: " + timeUntilRespawn + " (now: " + PhotonNetworkManager.time + ")");
                    valuesToSend.Add(pi.ViewID);
        ...
        this.photonView.RPC("PickupItemInit", targetPlayer, PhotonNetworkManager.time, valuesToSend.ToArray());
    }
    ...
    public void PickupItemInit(double timeBase, float[] inactivePickupsAndTimes)
    {
        ...
        for (int i = 0; i < inactivePickupsAndTimes.Length / 2; i++)
        {
            ...
            else
            {
                ...
                double timeBeforeRespawn = timeOfRespawn - PhotonNetworkManager.time;
                if (timeUntilRespawnBasedOnTimeBase <= 0)
                {
