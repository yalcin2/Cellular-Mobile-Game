...
/// </summary>
/// \ingroup publicApi
public static class PhotonNetworkManager
{
    ...
    /// </summary>
    internal static readonly PhotonMenuManager photonMono;
    ...
    public static ServerSettings PhotonServerSettings = (ServerSettings)Resources.Load(PhotonNetworkManager.serverSettingsAssetFile, typeof(ServerSettings));
    ...
    public static ServerConnection Server { get { return (PhotonNetworkManager.networkingPeer != null) ? PhotonNetworkManager.networkingPeer.Server : ServerConnection.NameServer; } }
    ...
    public static PhotonPlayer masterClient
    {
        get
        {
            if (offlineMode)
            {
                return PhotonNetworkManager.player;
            }
    ...
    public static bool autoJoinLobby
    {
        get
        {
            return PhotonNetworkManager.PhotonServerSettings.JoinLobby;
        }
        set
        {
            PhotonNetworkManager.PhotonServerSettings.JoinLobby = value;
        }
    }
    ...
    public static bool EnableLobbyStatistics
    {
        get
        {
            return PhotonNetworkManager.PhotonServerSettings.EnableLobbyStatistics;
        }
        set
        {
            PhotonNetworkManager.PhotonServerSettings.EnableLobbyStatistics = value;
        }
    }
    ...
    {
        get { return PhotonNetworkManager.networkingPeer.LobbyStatistics; }
        // only available to reset the state conveniently. done by state updates of PUN
        private set { PhotonNetworkManager.networkingPeer.LobbyStatistics = value; }
    }
    ...
    public static bool isMessageQueueRunning
    {
        ...
        set
        {
            if (value) PhotonMenuManager.StartFallbackSendAckThread();
    ...
    static PhotonNetworkManager()
    {
        ...
        PhotonMenuManager[] photonHandlers = GameObject.FindObjectsOfType(typeof(PhotonMenuManager)) as PhotonMenuManager[];
        if (photonHandlers != null && photonHandlers.Length > 0)
        {
            ...
            foreach (PhotonMenuManager photonHandler in photonHandlers)
            {
        ...
        photonMono = (PhotonMenuManager)photonGO.AddComponent<PhotonMenuManager>();
        ...
        ConnectionProtocol protocol = PhotonNetworkManager.PhotonServerSettings.Protocol;
    ...
    public static bool ConnectUsingSettings(string gameVersion)
    {
		...
		{
        	PhotonNetworkManager.logLevel = PhotonServerSettings.PunLogging;
		}

		// only apply Settings if logLevel is default ( see ServerSettings.cs), else it means it's been set programmatically
		if (PhotonNetworkManager.networkingPeer.DebugOut == DebugLevel.ERROR)
		{
        	PhotonNetworkManager.networkingPeer.DebugOut = PhotonServerSettings.NetworkLogging;
		}
    ...
    public static bool ConnectToBestCloudServer(string gameVersion)
    {
        ...
        if (PhotonServerSettings == null)
        {
            Debug.LogError("Can't connect: Loading settings failed. ServerSettings asset must be in any 'Resources' folder as: " + PhotonNetworkManager.serverSettingsAssetFile);
            return false;
        }

        if (PhotonServerSettings.HostType == ServerSettings.HostingOption.OfflineMode)
        {
            return PhotonNetworkManager.ConnectUsingSettings(gameVersion);
        }

        networkingPeer.IsInitialConnect = true;
        networkingPeer.SetApp(PhotonServerSettings.AppID, gameVersion);

        bool couldConnect = PhotonNetworkManager.networkingPeer.ConnectToNameServer();
        return couldConnect;
    }
    ...
    public static bool ConnectToRegion(CloudRegionCode region, string gameVersion)
    {
        ...
        if (PhotonServerSettings == null)
        {
            Debug.LogError("Can't connect: ServerSettings asset must be in any 'Resources' folder as: " + PhotonNetworkManager.serverSettingsAssetFile);
            return false;
        }
    ...
    {
        PhotonMenuManager.BestRegionCodeInPreferences = region;
    }
    ...
    {
        if (PhotonNetworkManager.connected && PhotonNetworkManager.Server == ServerConnection.MasterServer)
        {
            if (typedLobby == null)
            {
    ...
    public static bool LeaveRoom(bool becomeInactive = true)
    {
        ...
        else
        {
            if (room == null)
            {
                Debug.LogWarning("PhotonNetwork.room is null. You don't have to call LeaveRoom() when you're not in one. State: " + PhotonNetworkManager.connectionStateDetailed);
            }
            else
            {
    ...
    public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
    {
        ...
        {
            Debug.LogError("Failed to Instantiate prefab: " + prefabName + ". Client should be in a room. Current connectionStateDetailed: " + PhotonNetworkManager.connectionStateDetailed);
            return null;
        }
    ...
    public static GameObject InstantiateSceneObject(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
    {
        if (!connected || (InstantiateInRoomOnly && !inRoom))
        {
            Debug.LogError("Failed to InstantiateSceneObject prefab: " + prefabName + ". Client should be in a room. Current connectionStateDetailed: " + PhotonNetworkManager.connectionStateDetailed);
            return null;
        }
    ...
    public static void DestroyPlayerObjects(int targetPlayerId)
    {
        ...
        {
            Debug.LogError("DestroyPlayerObjects() failed, cause players can only destroy their own GameObjects. A Master Client can destroy anyone's. This is master: " + PhotonNetworkManager.isMasterClient);
        }
    }
    ...
    internal static void RPC(PhotonView view, string methodName, PhotonTargets target, bool encrypt, params object[] parameters)
    {
        ...
        if (networkingPeer != null)
        {
            if (PhotonNetworkManager.room.serverSideMasterClient)
            {
                networkingPeer.RPC(view, methodName, target, null, encrypt, parameters);
            }
            else
            {
                if (PhotonNetworkManager.networkingPeer.hasSwitchedMC && target == PhotonTargets.MasterClient)
                {
                    networkingPeer.RPC(view, methodName, PhotonTargets.Others, PhotonNetworkManager.masterClient, encrypt, parameters);
                }
                else
                {
                    networkingPeer.RPC(view, methodName, target, null, encrypt, parameters);
    ...
    public static void CacheSendMonoMessageTargets(Type type)
    {
        ...
        PhotonNetworkManager.SendMonoMessageTargets = FindGameObjectsWithComponent(type);
    }
    ...
    public static void LoadLevel(int levelNumber)
    {
        ...
        PhotonNetworkManager.isMessageQueueRunning = false;
    ...
    {
        PhotonNetworkManager.PhotonServerSettings = (ServerSettings)Resources.Load(PhotonNetworkManager.serverSettingsAssetFile, typeof(ServerSettings));
        ...
        if (PhotonNetworkManager.PhotonServerSettings == null)
        {
			string _PunResourcesPath = PhotonNetworkManager.FindPunAssetFolder();

			_PunResourcesPath += "Resources/";


			string serverSettingsAssetPath = _PunResourcesPath+ PhotonNetworkManager.serverSettingsAssetFile + ".asset";
            ...
            PhotonNetworkManager.PhotonServerSettings = (ServerSettings)ScriptableObject.CreateInstance("ServerSettings");
            if (PhotonNetworkManager.PhotonServerSettings != null)
            {
				AssetDatabase.CreateAsset(PhotonNetworkManager.PhotonServerSettings, serverSettingsAssetPath);
            }
            else
            {
