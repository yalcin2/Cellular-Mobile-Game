...
public class PhotonPlayer : IComparable<PhotonPlayer>, IComparable<int>, IEquatable<PhotonPlayer>, IEquatable<int>
{
    ...
    public string NickName 
    {
        ...
        set
        {
            ...
            PhotonNetworkManager.playerName = value;   // this will sync the local player's name in a room
        }
    }
    ...
    {
        get { return (PhotonNetworkManager.networkingPeer.mMasterClientId == this.ID); }
    }
    ...
    public void SetCustomProperties(Hashtable propertiesToSet, Hashtable expectedValues = null, bool webForward = false)
    {
        ...
        bool inOnlineRoom = this.actorID > 0 && !PhotonNetworkManager.offlineMode;
        ...
        if (inOnlineRoom)
        {
            PhotonNetworkManager.networkingPeer.OpSetPropertiesOfActor(this.actorID, customProps, customPropsToCheck, webForward);
        }
    ...
    public static PhotonPlayer Find(int ID)
    {
        ...
        {
            return PhotonNetworkManager.networkingPeer.GetPlayerWithId(ID);
        }
        return null;
    }
    ...
    {
        if (PhotonNetworkManager.networkingPeer == null || PhotonNetworkManager.networkingPeer.mActors == null || PhotonNetworkManager.networkingPeer.mActors.Count < 2)
        {
            return null;
        }

        Dictionary<int, PhotonPlayer> players = PhotonNetworkManager.networkingPeer.mActors;
