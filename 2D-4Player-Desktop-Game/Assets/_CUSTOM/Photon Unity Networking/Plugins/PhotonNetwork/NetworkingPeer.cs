...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
    ...
    {
        get { return string.Format("{0}_{1}", PhotonNetworkManager.gameVersion, PhotonNetworkManager.versionPUN); }
    }
    ...
    {
        get { return PhotonNetworkManager.EnableLobbyStatistics && this.Server == ServerConnection.MasterServer; }
    }
    ...
    public int mMasterClientId
    {
        get
        {
            if (PhotonNetworkManager.offlineMode) return this.LocalPlayer.ID;
    ...
    public NetworkingPeer(string playername, ConnectionProtocol connectionProtocol) : base(connectionProtocol)
    {
        ...
        rpcShortcuts = new Dictionary<string, int>(PhotonNetworkManager.PhotonServerSettings.RpcList.Count);
        for (int index = 0; index < PhotonNetworkManager.PhotonServerSettings.RpcList.Count; index++)
        {
            var name = PhotonNetworkManager.PhotonServerSettings.RpcList[index];
    ...
    private string GetNameServerAddress()
    {
        ...
        if (PhotonNetworkManager.UseAlternativeUdpPorts && this.TransportProtocol == ConnectionProtocol.Udp)
        {
            protocolPort = 27000;
        }
...
#region Operations and Connection Methods
    ...
    public bool Connect(string serverAddress, ServerConnection type)
    {
        ...
        {
            Debug.LogError("Connect() failed. Can't connect while disconnecting (still). Current state: " + PhotonNetworkManager.connectionStateDetailed);
            return false;
        }
    ...
    bool Reconnect()
    {
        ...
        PhotonNetworkManager.SwitchToProtocol(PhotonNetworkManager.PhotonServerSettings.Protocol);
    ...
    protected internal void SetupProtocol(ServerConnection serverType)
    {
        ...
        if (this.AuthMode == AuthModeOption.AuthOnceWss)
        {
            ...
            {
                if (PhotonNetworkManager.logLevel >= PhotonLogLevel.ErrorsOnly)
                {
                    Debug.LogWarning("Using PhotonServerSettings.Protocol when leaving the NameServer (AuthMode is AuthOnceWss): " + PhotonNetworkManager.PhotonServerSettings.Protocol);
                }
                protocolOverride = PhotonNetworkManager.PhotonServerSettings.Protocol;
            }
            else
            {
                if (PhotonNetworkManager.logLevel >= PhotonLogLevel.ErrorsOnly)
        ...
        if (PhotonPeer.NoSocket)
        {
            ...
            PhotonMenuManager.PingImplementation = typeof(PingMonoEditor);
            #endif
        ...
        if (PhotonMenuManager.PingImplementation == null)
        {
            PhotonMenuManager.PingImplementation = typeof(PingMono);
        }
    ...
    private bool CallAuthenticate()
    {
        ...
        {
            return this.OpAuthenticateOnce(this.AppId, this.AppVersion, auth, this.CloudRegion.ToString(), this.EncryptionMode, PhotonNetworkManager.PhotonServerSettings.Protocol);
        }
    }
    ...
    private void ReadoutProperties(Hashtable gameProperties, Hashtable pActorProperties, int targetActorNr)
    {
        ...
        if (this.CurrentRoom != null && gameProperties != null)
        {
            ...
            if (PhotonNetworkManager.automaticallySyncScene)
            {
    ...
    private void LeftRoomCleanup()
    {
        ...
        bool autoCleanupSettingOfRoom = (this.CurrentRoom != null) ? this.CurrentRoom.AutoCleanUp : PhotonNetworkManager.autoCleanUpPlayerObjects;
        ...
        if (autoCleanupSettingOfRoom)
        {
            ...
            PhotonNetworkManager.manuallyAllocatedViewIds = new List<int>();       // filled and easier to replace completely
        }
    ...
    protected internal void LocalCleanupAnythingInstantiated(bool destroyInstantiatedGameObjects)
    {
        ...
        PhotonNetworkManager.lastUsedViewSubId = 0;
        PhotonNetworkManager.lastUsedViewSubIdStatic = 0;
    }
        ...
        if (operationResponse.ReturnCode != 0)
        {
            switch (operationResponse.OperationCode)
            {
                case OperationCode.CreateGame:
                    if (PhotonNetworkManager.logLevel >= PhotonLogLevel.Informational)
                    {
    ...
    {
        SendMonoMessage(PhotonNetworkingMessage.OnMasterClientSwitched, PhotonNetworkManager.masterClient);
    }
    ...
    {
        if (PhotonNetworkManager.player != null)
        {
            return PhotonNetworkManager.player.AllProperties;
        }
    ...
    public void DebugReturn(DebugLevel level, string message)
    {
        ...
        else if (level == DebugLevel.INFO && PhotonNetworkManager.logLevel >= PhotonLogLevel.Informational)
        {
            Debug.Log(message);
        }
        else if (level == DebugLevel.ALL && PhotonNetworkManager.logLevel == PhotonLogLevel.Full)
        {
            Debug.Log(message);
        }
    }
    ...
    {
        if (PhotonNetworkManager.networkingPeer.State == ClientState.Disconnecting)
        {
            if (PhotonNetworkManager.logLevel >= PhotonLogLevel.Informational)
            {
        ...
        switch (operationResponse.OperationCode)
        {
            ...
            case OperationCode.AuthenticateOnce:
            {
                ...
                else
                {
                    ...
                    if (this.Server == ServerConnection.NameServer || this.Server == ServerConnection.MasterServer)
                    {
                        if (operationResponse.Parameters.ContainsKey(ParameterCode.UserId))
                        {
                            ...
                            if (!string.IsNullOrEmpty(incomingId))
                            {
                                ...
                                PhotonNetworkManager.player.UserId = incomingId;
                    ...
                    else if (this.Server == ServerConnection.MasterServer)
                    {
                        ...
                        if (PhotonNetworkManager.autoJoinLobby)
                        {
            ...
            case OperationCode.GetRegions:
                ...
                for (int i = 0; i < regions.Length; i++)
                {
                    ...
                    if (PhotonNetworkManager.PhotonServerSettings.HostType == ServerSettings.HostingOption.BestRegion && PhotonNetworkManager.PhotonServerSettings.EnabledRegions != 0)
                    {
                        CloudRegionFlag flag = Region.ParseFlag(code);
                        enabledRegion = ((PhotonNetworkManager.PhotonServerSettings.EnabledRegions & flag) != 0);
                        if (!enabledRegion && PhotonNetworkManager.logLevel >= PhotonLogLevel.Informational)
                        {
                ...
                if (PhotonNetworkManager.PhotonServerSettings.HostType == ServerSettings.HostingOption.BestRegion)
                {
					
			        CloudRegionCode bestFromPrefs = PhotonMenuManager.BestRegionCodeInPreferences;
				    ...
				    )
			        {
						...
						{
							PhotonMenuManager.PingAvailableRegionsAndConnectToBest();
						}
			        }else{
            ...
            case OperationCode.JoinRandomGame:
            {
                ...
                if (operationResponse.ReturnCode != 0)
                {
                    ...
                    {
                        if (PhotonNetworkManager.logLevel >= PhotonLogLevel.Full)
                            Debug.Log("JoinRandom failed: No open game. Calling: OnPhotonRandomJoinFailed() and staying on master server.");
                    }
                    else if (PhotonNetworkManager.logLevel >= PhotonLogLevel.Informational)
                    {
            ...
            case OperationCode.FindFriends:
                ...
                if (onlineList != null && roomList != null && this.friendListRequested != null && onlineList.Length == this.friendListRequested.Length)
                {
                    ...
                    PhotonNetworkManager.Friends = friendList;
                }
                else
                {
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        switch (photonEvent.Code)
        {
            case PunEvent.OwnershipRequest:
            {
                ...
                if (PhotonNetworkManager.logLevel == PhotonLogLevel.Informational)
                    Debug.Log("Ev OwnershipRequest " + requestedView.ownershipTransfer + ". ActorNr: " + actorNr + " takes from: " + currentOwner + ". local RequestedView.ownerId: " + requestedView.ownerId + " isOwnerActive: " + requestedView.isOwnerActive + ". MasterClient: " + this.mMasterClientId + ". This client's player: " + PhotonNetworkManager.player.ToStringFull());
                ...
                switch (requestedView.ownershipTransfer)
                {
                    ...
                    case OwnershipOption.Request:
                        if (currentOwner == PhotonNetworkManager.player.ID || PhotonNetworkManager.player.IsMasterClient)
                        {
                            if ((requestedView.ownerId == PhotonNetworkManager.player.ID) || (PhotonNetworkManager.player.IsMasterClient && !requestedView.isOwnerActive))
                            {
            ...
            case PunEvent.CloseConnection:
                ...
                {
                    PhotonNetworkManager.LeaveRoom(false);
                }
            ...
            case EventCode.ErrorInfo:
                if (PhotonNetworkManager.OnEventCall != null)
                {
                    object content = photonEvent[ParameterCode.Info];
                    PhotonNetworkManager.OnEventCall(photonEvent.Code, content, actorNr);
                }
                else
                {
    ...
    public static void SendMonoMessage(PhotonNetworkingMessage methodString, params object[] parameters)
    {
        ...
        if (PhotonNetworkManager.SendMonoMessageTargets != null)
        {
            objectsToCall = PhotonNetworkManager.SendMonoMessageTargets;
        }
        else
        {
            objectsToCall = PhotonNetworkManager.FindGameObjectsWithComponent(PhotonNetworkManager.SendMonoMessageTargetType);
        }
    ...
    protected internal void ExecuteRpc(Hashtable rpcData, int senderID = 0)
    {
        ...
        if (rpcData.ContainsKey((byte)5))
        {
            ...
            if (rpcIndex > PhotonNetworkManager.PhotonServerSettings.RpcList.Count - 1)
            {
            ...
            {
                inMethodName = PhotonNetworkManager.PhotonServerSettings.RpcList[rpcIndex];
            }
        }
        else
        {
        ...
        {
            int viewOwnerId = netViewID/PhotonNetworkManager.MAX_VIEW_IDS;
        ...
        int receivers = 0;
        int foundMethods = 0;
        if (!PhotonNetworkManager.UseRpcMonoBehaviourCache || photonNetview.RpcMonoBehaviours == null || photonNetview.RpcMonoBehaviours.Length == 0)
        {
                ...
                if (mInfo.Name.Equals(inMethodName))
                {
                    ...
                    if (pArray.Length == argTypes.Length)
                    {
                        ...
                        if (this.CheckTypeMatch(pArray, argTypes))
                        {
                            ...
                            if (PhotonNetworkManager.StartRpcsAsCoroutine && mInfo.ReturnType == typeof(IEnumerator))
                            {
    ...
    internal Hashtable SendInstantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, int[] viewIDs, object[] data, bool isGlobalObject)
    {
        ...
        instantiateEvent[(byte)6] = PhotonNetworkManager.ServerTimestamp;
    ...
    internal void RPC(PhotonView view, string methodName, PhotonTargets target, PhotonPlayer player, bool encrypt, params object[] parameters)
    {
        ...
        rpcEvent[(byte)2] = PhotonNetworkManager.ServerTimestamp;
    ...
    public void NewSceneLoaded()
    {
        if (this.loadingLevelAndPausedNetwork)
        {
            ...
            PhotonNetworkManager.isMessageQueueRunning = true;
        }
    ...
    {
        if (!PhotonNetworkManager.connected || PhotonNetworkManager.offlineMode || this.mActors == null)
        {
            return;
        }
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (view.synchronization == ViewSynchronization.ReliableDeltaCompressed || view.mixedModeIsReliable)
            {
                ...
                if (groupHashtable.Count >= NetworkingPeer.ObjectsInOneUpdate)
                {
                    ...
                    groupHashtable[(byte)0] = PhotonNetworkManager.ServerTimestamp;
                    if (this.currentLevelPrefix >= 0)
                    {
    ...
    private object[] OnSerializeWrite(PhotonView view)
    {
        ...
        PhotonMessageInfo info = new PhotonMessageInfo(this.LocalPlayer, PhotonNetworkManager.ServerTimestamp, view);
    ...
    bool AlmostEquals(object one, object two)
    {
        ...
        if (!one.Equals(two))
        {
            ...
            if (one is Vector3)
            {
                ...
                if (a.AlmostEquals(b, PhotonNetworkManager.precisionForVectorSynchronization))
                {
                    return true;
                }
            }
            ...
            else if (one is Quaternion)
            {
                ...
                if (a.AlmostEquals(b, PhotonNetworkManager.precisionForQuaternionSynchronization))
                {
                    return true;
                }
            }
            else if (one is float)
            {
                float a = (float)one;
                float b = (float)two;
                if (a.AlmostEquals(b, PhotonNetworkManager.precisionForFloatSynchronization))
                {
                    return true;
                }
            }
    ...
    {
		if (!PhotonNetworkManager.automaticallySyncScene || PhotonNetworkManager.isMasterClient || PhotonNetworkManager.room == null)
        {
            return;
        }
        ...
        if (!PhotonNetworkManager.room.CustomProperties.ContainsKey(NetworkingPeer.CurrentSceneProperty))
        {
            return;
        }

		// check if we are loading asynchronously
		bool LoadAsync = PhotonNetworkManager.room.CustomProperties.ContainsKey (NetworkingPeer.CurrentScenePropertyLoadAsync);

        // if loaded level is not the one defined my master in props, load that level
        object sceneId = PhotonNetworkManager.room.CustomProperties[NetworkingPeer.CurrentSceneProperty];
        if (sceneId is int)
        {
            if (SceneManagerHelper.ActiveSceneBuildIndex != (int)sceneId)
			{
				if (LoadAsync)
				{
					PhotonNetworkManager.LoadLevelAsync((int)sceneId);
				}else{
                	PhotonNetworkManager.LoadLevel((int)sceneId);
				}
			}
        }
        else if (sceneId is string)
        {
            if (SceneManagerHelper.ActiveSceneName != (string)sceneId)
			{
				if (LoadAsync)
				{
					PhotonNetworkManager.LoadLevelAsync((string)sceneId);
				}else{
                	PhotonNetworkManager.LoadLevel((string)sceneId);
				}
			}
        }
    }
	...
	protected internal void SetLevelInPropsIfSynced(object levelId, bool asyncLoading = false)
    {
		...
		if (!asyncLoading && PhotonNetworkManager.room.CustomProperties.ContainsKey(NetworkingPeer.CurrentSceneProperty))
        {
            object levelIdInProps = PhotonNetworkManager.room.CustomProperties[NetworkingPeer.CurrentSceneProperty];
        ...
        PhotonNetworkManager.room.SetCustomProperties(setScene);
    ...
    public void SetApp(string appId, string gameVersion)
    {
        ...
        {
            PhotonNetworkManager.gameVersion = gameVersion.Trim();
        }
    }
