...
public class PickupDemoGui : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        if (this.ShowScores)
        {
            GUILayout.Label("Your Score: " + PhotonNetworkManager.player.GetScore());
        }
        ...
        if (this.ShowDropButton)
        {
            foreach (PickupItem item in PickupItem.DisabledPickupItems)
            {
                if (item.PickupIsMine && item.SecondsBeforeRespawn <= 0)
                {
                    ...
                    GameObject playerCharGo = PhotonNetworkManager.player.TagObject as GameObject;
        ...
        if (this.ShowTeams)
        {
            ...
            {
                PhotonNetworkManager.player.SetTeam(PunTeams.Team.red);
            }
            if (GUILayout.Button("to blue"))
            {
                PhotonNetworkManager.player.SetTeam(PunTeams.Team.blue);
            }
        }
    }
}