...
public class RpsCore : PunBehaviour, IPunTurnManagerCallbacks
{
    ...
    private void UpdateScores()
    {
        ...
        {
            PhotonNetworkManager.player.AddScore(1);   // this is an extension method for PhotonPlayer. you can see it's implementation
        }
    }
    ...
    {
        PhotonPlayer remote = PhotonNetworkManager.player.GetNext();
        PhotonPlayer local = PhotonNetworkManager.player;
	...
	void RefreshUIViews()
	{
		...
		ConnectUiView.gameObject.SetActive(!PhotonNetworkManager.inRoom);
		GameUiView.gameObject.SetActive(PhotonNetworkManager.inRoom);

		ButtonCanvasGroup.interactable = PhotonNetworkManager.room!=null?PhotonNetworkManager.room.PlayerCount > 1:false;
	}
