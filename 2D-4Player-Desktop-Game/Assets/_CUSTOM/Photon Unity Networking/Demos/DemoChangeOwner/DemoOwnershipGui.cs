...
public class DemoOwnershipGui : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        {
            int playerNr = PhotonNetworkManager.player.ID;
            string playerIsMaster = PhotonNetworkManager.player.IsMasterClient ? "(master) " : "";
            string playerColor = this.GetColorName(PhotonNetworkManager.player.ID);
            GUILayout.Label(string.Format("player {0}, {1} {2}(you)", playerNr, playerColor, playerIsMaster));

            foreach (PhotonPlayer otherPlayer in PhotonNetworkManager.otherPlayers)
            {
            ...
            if (PhotonNetworkManager.inRoom && PhotonNetworkManager.otherPlayers.Length == 0)
            {
