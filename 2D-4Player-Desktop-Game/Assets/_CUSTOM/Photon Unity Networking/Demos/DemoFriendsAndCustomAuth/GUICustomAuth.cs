...
public class GUICustomAuth : MonoBehaviour
{
    ...
    public void ConnectWithNickname()
    {
        ...
        PhotonNetworkManager.AuthValues = new AuthenticationValues() {UserId = PhotonNetworkManager.playerName }; // null by default but maybe set in a previous session.
        PhotonNetworkManager.playerName = PhotonNetworkManager.playerName + "Nick";
        PhotonNetworkManager.ConnectUsingSettings("1.0");
        ...
        switch (guiState)
        {
            ...
            case GuiState.AuthInput:
                ...
                if (GUILayout.Button("Authenticate"))
                {
                    ...
                    PhotonNetworkManager.AuthValues.AuthType = CustomAuthenticationType.Custom;    

                    // the demo authentication-service expects values for "username" and "token":
                    PhotonNetworkManager.AuthValues.AddAuthParameter("username", this.authName);
                    PhotonNetworkManager.AuthValues.AddAuthParameter("token", this.authToken);
