...
public class PhotonEditor : EditorWindow
{
    ...
    private static void OnUpdate()
    {
        ...
        if (!postCompileActionsDone && !EditorApplication.isCompiling && !EditorApplication.isPlayingOrWillChangePlaymode && PhotonNetworkManager.PhotonServerSettings != null)
        {
	...
	#endif
    {
        ...
        if (PhotonNetworkManager.PhotonServerSettings.HostType == ServerSettings.HostingOption.NotSet)
        {
    ...
    protected virtual void UiSetupApp()
    {
        ...
        if (GUILayout.Button(CurrentLang.SetupButton, GUILayout.Width(100)))
        {
            ...
            {
                this.photonSetupState = PhotonSetupStates.GoEditPhotonServerSettings;
                Undo.RecordObject(PhotonNetworkManager.PhotonServerSettings, "Update PhotonServerSettings for PUN");
                PhotonNetworkManager.PhotonServerSettings.UseCloud(this.mailOrAppId);
                PhotonEditor.SaveSettings();
            }
        }
    ...
    protected virtual void RegisterWithEmail(string email)
    {
        ...
        if (client.ReturnCode == 0)
        {
            ...
            PhotonNetworkManager.PhotonServerSettings.UseCloud(this.mailOrAppId, 0);
            if (PhotonEditorUtils.HasVoice)
            {
                PhotonNetworkManager.PhotonServerSettings.VoiceAppID = client.AppId2;
            }
            PhotonEditor.SaveSettings();
        ...
        {
            PhotonNetworkManager.PhotonServerSettings.HostType = ServerSettings.HostingOption.PhotonCloud;
            PhotonEditor.SaveSettings();
    ...
    {
        Selection.objects = new UnityEngine.Object[] { PhotonNetworkManager.PhotonServerSettings };
        EditorGUIUtility.PingObject(PhotonNetworkManager.PhotonServerSettings);
    }
    ...
    {
        EditorUtility.SetDirty(PhotonNetworkManager.PhotonServerSettings);
    }
    ...
    public static void UpdateRpcList()
    {
        ...
        foreach (var mono in types)
        {
            ...
            foreach (MethodInfo method in methods)
            {
                ...
                if (isOldRpc || method.IsDefined(typeof(PunRPC), false))
                {
                    ...
                    if (!additionalRpcs.Contains(method.Name) && !PhotonNetworkManager.PhotonServerSettings.RpcList.Contains(method.Name))
                    {
        ...
        {
            // LIMITS RPC COUNT
            if (additionalRpcs.Count + PhotonNetworkManager.PhotonServerSettings.RpcList.Count >= byte.MaxValue)
            {
                if (currentRpcs.Count <= byte.MaxValue)
                {
                    ...
                    if (clearList)
                    {
                        PhotonNetworkManager.PhotonServerSettings.RpcList.Clear();
                        PhotonNetworkManager.PhotonServerSettings.RpcList.AddRange(currentRpcs);
                    }
                    else
                    {
                        return;
            ...
            additionalRpcs.Sort();
            Undo.RecordObject(PhotonNetworkManager.PhotonServerSettings, "Update PUN RPC-list");
            PhotonNetworkManager.PhotonServerSettings.RpcList.AddRange(additionalRpcs);
            PhotonEditor.SaveSettings();
        }
