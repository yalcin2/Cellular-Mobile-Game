...
public class ServerSettingsInspector : Editor
{
	...
	private void OnUpdate()
	{
	    ...
	    if (Application.isPlaying && this.lastServer != PhotonNetworkManager.Server)
	    {
	        this.lastServer = PhotonNetworkManager.Server;
	        Repaint();
	    }
	}
    ...
    public override void OnInspectorGUI()
    {
        ...
        switch (settings.HostType)
        {
            ...
            case ServerSettings.HostingOption.PhotonCloud:
		...
		else // Bestregion
                {
					...
					if (PhotonNetworkManager.connected && PhotonNetworkManager.Server != ServerConnection.NameServer)
					{
					    _regionFeedback = "Current:" + PhotonNetworkManager.CloudRegion + " " + _regionFeedback;
					}
		...
		if (EditorApplication.isPlaying && PhotonNetworkManager.logLevel!=_PunLogging)
		{
			PhotonNetworkManager.logLevel = _PunLogging;
		}
    ...
    private int RpcListHashCode()
    {
        ...
        int hashCode = PhotonNetworkManager.PhotonServerSettings.RpcList.Count + 1;
        foreach (string s in PhotonNetworkManager.PhotonServerSettings.RpcList)
        {
